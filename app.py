...
def fetch_poster(movie_id):
    try:
        ...
        url = 'https://api.themoviedb.org/3/movie/{}?api_key=f32854651b05c6bdb2d20473a1ac108e&language=en-US'.format(movie_id)
        ...
        poster_path = data.get('poster_path')  # Extracting poster path from response
        if poster_path:
            return "https://image.tmdb.org/t/p/w500/" + poster_path  # Constructing full poster URL
        else:
...
def recommend(movie):
    movie_index = movies[movies['title'] == movie].index[0]  # Getting index of selected movie
    distances = similarity[movie_index]  # Getting similarity scores for the selected movie
    # Sort the distances in descending order
    movies_list = sorted(list(enumerate(distances)), reverse=True, key=lambda x: x[1])
    print("All movies with similarity scores:")
    for i, (index, score) in enumerate(movies_list):
        print(f"{movies.iloc[index].title}: {score}")  # Printing movie titles and their similarity scores
    recommend_movies = []
    ...
    for i, (index, score) in enumerate(movies_list):
        ...
        recommend_movies_posters.append(fetch_poster(movie_id))  # Fetching poster for recommended movie
        count += 1
        if count == 5:
...
movies_dict = pickle.load(open('movie_recomm/app_code/movie_dict.pkl', 'rb'))
similarity = pickle.load(open('movie_recomm/app_code/similarity.pkl', 'rb'))
...
if st.button('Recommend'):
    names, posters = recommend(selected_movie_name)  # Getting recommendations for the selected movie
